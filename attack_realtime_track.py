...
def run(
    ...
    # yellow_image = np.full((480, 640, 3), (100, 0, 255), dtype=np.uint8)
    # yellow_image = np.transpose(yellow_image,(2,0,1))
    # yellow_image = yellow_image.reshape(1, 3, 480, 640)
    # # # print(yellow_image.shape)
    # perturbation = yellow_image*1.0
    learning_rate = 0.1
    import matplotlib.pyplot as plt
    import time
    from PIL import Image
    perturbation = np.ones((1, 3, 480, 640))
    for path, im, im0s, vid_cap, s in dataset:
        perturbation = np.ones((1, 3, 480, 640))
        # 创建一个全白图像，形状与 im 相同
        # white_image = np.zeros_like(im) * 255
        # 将 im 左上角 (200, 200) 的部分替换为全白图像的对应部分
        # im[:, :, :200, :200] = white_image[:, :, :200, :200]
        # im = np.zeros_like(im) + im

        perturbation = torch.tensor(perturbation, requires_grad=True)
        perturbed_image = torch.tensor(im, dtype=torch.float32) + perturbation
        im = perturbed_image
        # im = torch.clamp(perturbed_image, 0, 255)
        im_transposed = np.transpose(im.detach().numpy(), (0, 2, 3, 1))
        ...
        with dt[1]:
            ...
            if model.xml and im.shape[0] > 1:
                ...
                # # 将图像的形状调整为 (480, 640, 3)，以便正确显示
                # im_reshaped = np.transpose(im.cpu().detach().numpy()[0], (1, 2, 0))
                #
                # # 将numpy数组转换为PIL.Image对象
                # image = Image.fromarray((im_reshaped*255).astype(np.uint8))
                #
                # # 保存图像为图片文件
                ...
                # print("1",len(pred[1]))
                # print("11", len(pred[1][0][0]))
                # print("12", len(pred[1][0][1]))
                # print("13", len(pred[1][0][2]))
                # all                2
                # 0                torch.Size([1, 18900, 85]) batchsize,预测框数量，属性数量
                # 1                3
                # 11                1
                # 12                1
                # 13                1
        ...
        # PERSON
        # flag = ((pred[0][0][:, 5] > 0.2) * 1)
        # loss_yolo = ((flag * pred[0][0][:, 5] )).sum()
        # loss = -loss_yolo
        # loss.requires_grad_(True)


        # all
        # flag = ((pred[0][0][:, 4] > 0.5) * 1)
        # loss_yolo = ((flag * pred[0][0][:, 4] )).sum()
        # loss = loss_yolo
        # loss.requires_grad_(True)


        # flag = ((pred[0][0][:, 5] > 0.5) * 1)
        # loss_yolo = ((flag * pred[0][0][:, 4])).sum()
        # loss = -loss_yolo
        # loss.requires_grad_(True)

        flag_person = ((pred[0][0][:, 12] > 0.2) & (pred[0][0][:, 4] > 0.5)) * 1

        loss_yolo_person = ((flag_person * pred[0][0][:, 4])).sum()

        flag_others = ((pred[0][0][:, 12] < 0.5) & (pred[0][0][:, 4] > 0.8)) * 1
        ...
        # print(perturbation_grad.shape)torch.Size([1, 3, 480, 640])
        # print("perturbation.grad",perturbation_grad)
        # 更新perturbation
        # with torch.no_grad():
        #     perturbation += learning_rate * perturbation_grad
        # perturbation.add_(learning_rate * perturbation_grad)
        # 假设box的形状为[1, 3, 480, 640]


        # 创建一个与perturbation_grad形状相同的全零张量
        ...
        if len(boxes) > 0:
            print("满足条件的boxes:", boxes)

            for i in range(len(boxes)):
                box = xywh2xyxy(boxes[i])
                print("box", box)
                print(perturbation_grad_all.shape)
                print(int(box[0]),int(box[2]),
                int(box[1]),int(box[3]))
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if webcam:  # batch_size >= 1
                ...
                im0 = np.transpose(im.cpu().detach().numpy(), (0, 2, 3, 1))[0]*255
            ...
            if view_img:
                ...
                cv2.imshow("Output Image", perturbation_output)
                cv2.imshow(str(p), im0)

                # cv2.imwrite("images/output_image.jpg", im0)
                # cv2.imwrite("images/per.jpg", perturbation_output)
            ...
            if save_img:
                if dataset.mode == "image":
                    ...
                    im0_8u = cv2.convertScaleAbs(im0/255, alpha=(255.0))

                    vid_writer[i].write(im0_8u)
