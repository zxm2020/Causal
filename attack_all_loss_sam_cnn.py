...
# 假设我们有一个大模型 (Fusion Model)
class FusionModel(nn.Module):
    def __init__(self):
        super(FusionModel, self).__init__()
        # 假设大模型有一些卷积层或其他网络结构来融合图像与扰动
        self.conv1 = nn.Conv2d(6, 64, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU()

    def forward(self, im, perturbation):
        # 将图像和扰动拼接作为输入
        # 确保 im 和 perturbation 都是 tensor，并且需要计算梯度
        # im = im.clone().detach().requires_grad_(True)  # 确保图像 im 是 tensor 并且需要计算梯度
        # perturbation = perturbation.clone().detach().requires_grad_(True)  # 确保扰动 perturbation 是 tensor 并且需要计算梯度

        # 拼接图像和扰动
        x = torch.cat((torch.tensor(im), perturbation), dim=1)  # 拼接成6个通道
        x = self.relu(self.conv1(x.to(torch.float32)))  # 卷积层1
        fusion_im = self.conv2(x)  # 卷积层2，输出融合图像
        return fusion_im
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        # im(1, 3, 480, 640)
        # im0s(1, 480, 640, 3)
        physical_perturbed_image = torch.tensor(im)
        camera_im = im
        perturbation = np.ones((1, 3, 480, 640))*128/255
        digital_perturbed_image = physical_perturbed_image.to(torch.float32)
        for epoch in range(10):

            perturbation = torch.tensor(perturbation, requires_grad=True)

            # print("1",perturbation)
            perturbed_image = fusion_model(im, perturbation)
            # perturbed_image = perturbation

            loss_fusion = torch.dist(digital_perturbed_image, physical_perturbed_image, p=2)
            # loss_fusion = torch.nn.functional.mse_loss(perturbed_image, im)
            print("loss_fusion",loss_fusion)
            ...
            digital_perturbed_image = perturbed_image
            # Sam
